...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!provider || provider === 'email') return res.status(403).json({ error: 'not_social_account' });
    const identities = user.identities || [];
    if (identities.some(i => i.provider === 'email')) return res.status(403).json({ error: 'has_email_identity' });

    // 3. 생성 24시간 이내
    const created = Date.parse(user.created_at || '') || 0;
    if (!created || Date.now() - created > 24 * 60 * 60 * 1000) return res.status(403).json({ error: 'account_too_old' });
    ...
    for (const table of ['cats', 'user_settings']) {
      ...
      if (!q.ok) return res.status(500).json({ error: 'data_check_failed' });
      const rows = await q.json();
      if (Array.isArray(rows) && rows.length > 0) return res.status(403).json({ error: 'account_not_empty' });
    }
    ...
    if (!dRes.ok) return res.status(500).json({ error: 'delete_failed' });
